...
import { subscriptionsRevenueByMonth } from '../../../data/mockDashboard';
...
export function SubscriptionsRevenueChartWidget() {
  ...
  const { ticks, maxValue, plotW, plotH } = useMemo(() => {
    const totals = subscriptionsRevenueByMonth.map((m) => m.low + m.medium + m.high);
  ...
  const slotW = plotW / subscriptionsRevenueByMonth.length;
    ...
    <Card className="widget-risks-chart">
      ...
      <div className="widget-risks-chart__plot" ref={plotRef}>
        ...
        >
          ...
          {subscriptionsRevenueByMonth.map((row, i) => {
